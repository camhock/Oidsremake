...
scene.onHitWall(SpriteKind.Player, function (sprite, location) {
    if (ship.isHittingTile(CollisionDirection.Bottom) && facing == 0 && (velocityx < 11 && velocityx > -11 && velocityy < 11)) {
        velocityx = 0
        velocityy = 0
        shipcontrolled = "none"
    } else {
...
function callStart () {
    ...
    statusbar2 = statusbars.create(10, 10, StatusBarKind.Magic)
...
scene.onOverlapTile(SpriteKind.Player, assets.tile`tile2`, function (sprite, location) {
    totalsaved = saved3 + totalsaved
    game.showLongText("Prisoners saved this level :" + saved3, DialogLayout.Bottom)
    game.showLongText("Percentage saved this level " + saved3 / (levelbases * 2) * 100, DialogLayout.Bottom)
...
controller.A.onEvent(ControllerButtonEvent.Repeated, function () {
    ...
    if (facing == 0) {
        cannon = sprites.createProjectileFromSprite(assets.image`bULLET`, ship, 0, -100)
    } else if (facing == 1) {
        cannon = sprites.createProjectileFromSprite(assets.image`bULLET`, ship, 50, -50)
    } else if (facing == 2) {
        cannon = sprites.createProjectileFromSprite(assets.image`bULLET`, ship, 100, 0)
    } else if (facing == 3) {
        cannon = sprites.createProjectileFromSprite(assets.image`bULLET`, ship, 50, 50)
    } else if (facing == 4) {
        cannon = sprites.createProjectileFromSprite(assets.image`bULLET`, ship, 0, 100)
...
function clearLevel () {
    ...
    for (let value2 of sprites.allOfKind(SpriteKind.saved)) {
        value2.destroy()
    }
    for (let value3 of sprites.allOfKind(SpriteKind.tractor)) {
        value3.destroy()
    }
    for (let value4 of sprites.allOfKind(SpriteKind.prisoner)) {
        value4.destroy()
    }
    for (let value5 of sprites.allOfKind(SpriteKind.jail)) {
        value5.destroy()
    }
    for (let value6 of sprites.allOfKind(SpriteKind.enemybase)) {
        value6.destroy()
    }
    ship.destroy()
    mothership.destroy()
...
sprites.onDestroyed(SpriteKind.Player, function (sprite) {
    music.siren.play()
    prisonersleft = prisonersleft - saved3
    info.changeLifeBy(-1)
    scene.cameraFollowSprite(mothership)
    saved3 = 0
    shipcontrolled = "mother"
...
game.onUpdate(function () {
    ...
    if (shipcontrolled == "ship") {
        ...
        statusbar.value = saved3 / prisonersleft * 100
        if (velocityx > -11 && velocityx < 11 && velocityy < 11) {
            if (facing == 0) {
